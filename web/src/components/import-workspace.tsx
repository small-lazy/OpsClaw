...
type QueueItem = { id: string; file: File; status: "waiting" | "uploading" | "completed" | "failed"; progress: number; error?: string; validationError?: boolean; datasetIds?: string[] };
...
export function ImportWorkspace({ notify, go }: Props) {
  ...
  function add(files: FileList | File[]) {
    const entries = Array.from(files).map(file => {
      ...
      return { id: crypto.randomUUID(), file, status: error ? "failed" as const : "waiting" as const, progress: 0, error, validationError: Boolean(error) };
    });
