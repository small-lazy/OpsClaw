...
因子的自动发现条件、证据核对方法、人工审核与监控状态详见 [因子如何发现、判断与监控](FACTORS.md)。

需要使用模型进一步解释数据或编写规划时，在 [Agent 工作台](AGENTS.md) 配置模型连接，并为 Agent 授予增长汇总或因子读取工具。模型输出保存在运行记录中，具体广告动作仍通过本页的方案审核流程执行。
